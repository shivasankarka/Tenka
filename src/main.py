...
class PackageManager:
    ...
    def download_package(self, repo_url, package_name, branch='main'):
        ...
        if response.status_code == 200:
            package_path = os.path.join(self.home_dir, f"{package_name}.zip")
    ...
    def install_package(self, package_name, branch=False):
        ...
        if search_results:
            ...
            branch_path = os.path.join(self.home_dir, "envs", active_env, "lib", "mojo")
...
def main():
    parser = argparse.ArgumentParser(
        description="Tenka Package Manager CLI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  tenka create myenv         Create a new environment named 'myenv'
  tenka activate myenv       Activate the 'myenv' environment
  tenka search numojo        Search for the 'numojo' package in github
  tenka package numojo src   Install the 'numojo' package in the current environment from the src folder in current directory
        """
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Create environment
    parser_create = subparsers.add_parser('create', help='Create a new environment')
    parser_create.add_argument("env_name", help="Name of the environment to create")

    # Remove environment
    parser_remove = subparsers.add_parser('remove', help='Remove an existing environment')
    parser_remove.add_argument("env_name", help="Name of the environment to remove")
    
    # Activate environment
    parser_activate = subparsers.add_parser('activate', help='Activate an environment')
    parser_activate.add_argument("env_name", help="Name of the environment to activate")

    # Deactivate environment
    parser_deactivate = subparsers.add_parser('deactivate', help='Deactivate the current environment')
    
    # Current environment
    parser_current = subparsers.add_parser('current', help='Display the current active environment')

    # Install packages
    parser_install = subparsers.add_parser('install', help='Install packages in the current environment')
    parser_install.add_argument("package_name", help="Name of the package to install")
    parser_install.add_argument("--branch", action='store_true', help="Search for specific branches")

    # Search package
    parser_search = subparsers.add_parser('search', help='Search for a package')
    parser_search.add_argument("package_name", help="Name of the package to search for")

    # Uninstall package
    parser_uninstall = subparsers.add_parser('uninstall', help='Uninstall a package from the current environment')
    parser_uninstall.add_argument("package_name", help="Name of the package to uninstall")

    # package 
    parser_package = subparsers.add_parser('package', help='Package a folder')
    parser_package.add_argument("source_name", help="Name of the source folder to package")
    parser_package.add_argument("package_name", help="Name of the package")
    parser_package.add_argument("--package_path", help="Path of the source folder to package", required=False)
